...
function SummaryTable({ summaries }) {
  ...
  const formatNumber = (val, type = "currency") => {
    const num = Number(String(val || 0).replace(/,/g, ""));
    if (type === "currency") return `Rs. ${num.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
    if (type === "percentage") return `${num.toFixed(2)}%`;
    if (type === "number") return num.toLocaleString(undefined, { maximumFractionDigits: 4 });
    return val ?? "";
  };
                ...
                {SUMMARY_HEADERS.map((col, colIndex) => {
                  ...
                  switch (col) {
                    ...
                    case "XD/XB":
                      ...
                      break;
                    case "Shares":
                      content = formatNumber(item.shares, "number");
                      break;
                    case "Cost per share":
                      content = formatNumber(item.avgCost, "currency");
                      break;
                    case "Cumulative Cost":
                      content = formatNumber(item.cumulativeCost, "currency");
                      break;
                    case "Last Price":
                      content = renderLastPrice(item);
                      break;
                    ...
                    case "Unrealized Gain/Loss": {
                      const lastPriceNum = Number(String(item.lastPrice || 0).replace(/,/g, ""));
                      const avgCostNum = Number(String(item.avgCost || 0).replace(/,/g, ""));
                      const sharesNum = Number(String(item.shares || 0).replace(/,/g, ""));
                      const profit = (lastPriceNum - avgCostNum) * sharesNum;
                      const color = profit > 0 ? "green" : profit < 0 ? "red" : "gray";
                      content = <span style={{ color }}>Rs. {profit.toFixed(2)}</span>;
                      break;
                    }
                    case "Realized Gain/Loss": {
                      const realized = Number(String(item.realizedPnL || 0).replace(/,/g, ""));
                      const color = realized > 0 ? "green" : realized < 0 ? "red" : "gray";
                      content = <span style={{ color }}>Rs. {realized.toFixed(2)}</span>;
                      break;
                    }
                    case "Dividend Income": {
                      const div = Number(String(item.dividendIncome || 0).replace(/,/g, ""));
                      const color = div > 0 ? "green" : div < 0 ? "red" : "gray";
                      content = <span style={{ color }}>Rs. {div.toFixed(2)}</span>;
                      break;
                    }
                    case "Portfolio %":
                      content = formatNumber(item.portfolioPercent, "percentage");
                      break;
                    case "JSON":
                      content = (
                        <pre style={{
