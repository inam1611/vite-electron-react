...
//                   <button
//                     className="delete-btn"
//                     onClick={() => onDelete(txn)} // pass the whole txn object
//                   >
//                     Delete
//                   </button>
//                 </td>
...
function TransactionTable({ transactions, portfolio, onDelete }) {
  ...
  const filteredTransactions = useMemo(() => {
    return processedTransactions.filter((txn) =>
      Object.entries(filters).every(([col, allowedValues]) => {
        if (!allowedValues || allowedValues.length === 0) return true;
        let value = txn[col] ?? txn[col.toLowerCase()] ?? "";
        if (col === "Stock Symbol") value = (txn["Stock Symbol"] || txn.stockName || "").toUpperCase();
        return allowedValues.includes(value);
              ...
              return (
                <tr key={index}>
                  ...
                  {derivedColumns.map((col) => {
                    ...
                    if (format === "currency") {
                      const v = Number(rawValue || 0);
                      displayValue = `Rs. ${v.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                    } else if (format === "percentage") {
                      displayValue = `${Number(rawValue || 0).toFixed(2)} %`;
                    } else if (format === "number") {
                      displayValue = Number(rawValue || 0).toLocaleString(undefined, { maximumFractionDigits: 4 });
                    } else {
                      displayValue = rawValue ?? "";
                    }
                  ...
                  <td>
                    <button className="delete-btn" onClick={() => onDelete(txn)}>
                      Delete
                    </button>
                  </td>
                </tr>
